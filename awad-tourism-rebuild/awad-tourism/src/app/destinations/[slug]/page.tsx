...
export default function DestinationPage({ params }: { params: { slug?: string } }) {
  ...
  const displayBase = String(destination?.title ?? destination?.name ?? destination?.label ?? slug);
  ...
  const programs = (getPrograms() ?? []).filter((p: any) =>
    Array.isArray(p.destinationSlugs) ? p.destinationSlugs.includes(slug) : false
  );
    ...
    <div className="space-y-8">
      <div>
        ...
        <p className="mt-2 text-gray-600">
          Beautiful places, good prices, and flexible itineraries. Book fast via WhatsApp.
        </p>
      </div>
      ...
      {programs.length === 0 && (
        <div className="rounded-2xl border p-6 text-sm text-gray-600">
          No programs are linked to this destination yet. Add it in <code>data/programs.json</code>.
          <div className="mt-3">
